...
module.exports.getCredentialsFromToken = function (event, updateCredentials) {
    ...
    if (bearerToken) {
        ...
        if (!(tokenValue in tokenCache)) {
            ...
            var user_name = decodedIdToken['cognito:username'];
            console.log("--decodIDctoken----"+JSON.stringify(decodedIdToken))
            console.log("--userName----"+JSON.stringify(user_name))

            async.waterfall([
                function(callback) {
                    console.log("----qwqw--")
                    getUserPoolWithParams(user_name, callback)
                },
 ...
 */
function getUserPoolWithParams(user_name, callback) {
...
function authenticateUserInPool(userPool, idToken, callback) {
    ...
    console.log("--i+dToken----"+JSON.stringify(idToken))
    var params = {
        token: idToken,
        provider: provider,
        IdentityPoolId: userPool.identity_pool_id
    }
    var getIdentity = getId(params, function (err, data) {
        console.log("--data--qqq---"+JSON.stringify(data))
        if (err) {
