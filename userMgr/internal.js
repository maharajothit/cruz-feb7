...
class UserInternals {
     ...
     */
    lookupPool(event) {
        console.log("check internaljs---2----"+JSON.stringify(event))
        ...
        return new Promise(function (resolve, reject) {
            tokenManager.getSystemCredentials(function (err, credentials) {
                console.log("check internal js -3--"+JSON.stringify(credentials))
                if (err) {
                ...
                sharedFunctions.lookupUserPoolData(credentials, user_name, null, true, function (err, user) {
                    console.log("check internal js --4-"+JSON.stringify(user))
                    if (err) {
    ...
    reg(event) {
       ...
       return new Promise(function (resolve, reject) {
            ...
            tokenManager.getSystemCredentials(function (err, systemCredentials) {
                ...
                credentials = systemCredentials;
                console.log("systemCredentials---"+JSON.stringify(systemCredentials))
                ...
                provisionAdminUserWithRoles(user, credentials, configuration.userRole.tenantAdmin, configuration.userRole.tenantUser,
                    function (err, result) {
                        console.log("---provisionadmin res--"+console.log(result))
                        if (err) {
...
function provisionAdminUserWithRoles(user, credentials, adminPolicyName, userPolicyName, callback) {
    console.log("paur---1--"+JSON.stringify(user))
    ...
    console.log("--paur-2-policy param-"+JSON.stringify(policyCreationParams))
    ...
    //sharedFunctions.lookupUserPoolData(credentials, user.userName, user.tenant_id, true, function(err, userPoolData) {
      sharedFunctions.lookupUserPoolData(credentials, user.email, user.tenant_id, true, function(err, userPoolData) {
        console.log("---paur--userPoolData-"+JSON.stringify(userPoolData))
        if (!err){
           ...
           cognitoUsers.createUserPool(user.tenant_id)
                ...
                .then(function (adminPolicy) {
                    ...
                    console.log("Call createNewUser..."+JSON.stringify(user));
                ...
                .then(function(trustPolicyRole) {
                    ...
                    console.log("Call addPolicyToRole(admin)..."+JSON.stringify(adminPolicyRoleParams));
